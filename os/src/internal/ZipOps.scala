...
trait ZipOps { self =>
  object zip {
    ...
    private def makeZipEntry(
      ...
      val mode = PermissionUtils.modeFromPermissions(
        os.perms(file).toSet(),
        PermissionUtils.FileType.of(file.toNIO)
      )
  ...
  object unzip {
    ...
    private def isSymLink(mode: Int): Boolean =
      (mode & S_IFMT) == UnixStat.LINK_FLAG
    ...
    def streamRaw(
        ...
        excludePatterns: Seq[Regex] = List(),
        includePatterns: Seq[Regex] = List()
    ): geny.Generator[(juz.ZipEntry, java.io.InputStream)] = {
      new Generator[(juz.ZipEntry, java.io.InputStream)] {
        override def generate(handleItem: ((juz.ZipEntry, java.io.InputStream)) => Generator.Action)
            : Generator.Action = {
          var lastAction: Generator.Action = Generator.Continue
          source.readBytesThrough { inputStream =>
            val zipInputStream = new juz.ZipInputStream(inputStream)
            try {
              var zipEntry: juz.ZipEntry = zipInputStream.getNextEntry
              while (lastAction == Generator.Continue && zipEntry != null) {
                // Skip files that match the exclusion patterns
                if (self.zip.shouldInclude(zipEntry.getName, excludePatterns, includePatterns)) {
